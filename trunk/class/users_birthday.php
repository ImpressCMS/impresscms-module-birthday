...
if (!class_exists('Birthday_XoopsPersistableObjectHandler')) {
	include_once ICMS_ROOT_PATH.'/modules/birthday/class/PersistableObjectHandler.php';
}

class users_birthday extends Birthday_Object
{
...
class BirthdayUsers_birthdayHandler extends Birthday_XoopsPersistableObjectHandler
{
	...
	function getForm(users_birthday $item, $baseurl, $withUserSelect = true, $captcha = '')
	{
	    ...
	    require_once ICMS_ROOT_PATH.'/modules/birthday/class/formtextdateselect.php';
        $edit = true;
        if($item->isNew()) {
            $edit = false;
        }
        ...
        $sform->addElement(new BD_XoopsFormTextDateSelect(_BIRTHDAY_DATE, 'birthday_date', 15, $date));
	...
	function saveUser($withCurrentUser = false)
	{
		...
		$return = birthday_utils::uploadFile(0);
		if($return === true) {
			$newDestName = birthday_utils::createUploadName(birthday_utils::getModuleOption('folder_path'), basename($destname), true);
			$retval = birthday_utils::resizePicture(birthday_utils::getModuleOption('folder_path').DIRECTORY_SEPARATOR.$destname, birthday_utils::getModuleOption('folder_path').DIRECTORY_SEPARATOR.$newDestName, $images_width, $images_height);
